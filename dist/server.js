...
export function createMcpServer() {
    ...
    server.tool("list_endpoints", "List all available SSL Wireless ISMSPLUS API v3 endpoints with a brief description of each.", {}, async () => {
        const lines = [
            `SSL Wireless ISMSPLUS API v3 — Base URL: ${BASE_URL}`,
            "",
            ...Object.values(ENDPOINTS).map((ep) => `• ${ep.name} [${ep.methods.join("/")}] ${ep.path}\n  ${ep.description}`),
        ];
        return { content: [{ type: "text", text: lines.join("\n") }] };
    });
    // ── Tool 2: Get full endpoint spec ─────────────────────────────────────────
    server.tool("get_endpoint_spec", "Get the full parameter specification, constraints, and response schema for a specific ISMSPLUS API endpoint.", {
        endpoint: z
            .enum(ENDPOINT_IDS)
    ...
    server.tool("generate_integration_code", "Generate a ready-to-adapt code snippet for integrating a specific ISMSPLUS endpoint in your chosen programming language.", {
        endpoint: z
            .enum(ENDPOINT_IDS)
    ...
    server.tool("explain_error_code", "Explain an ISMSPLUS API error or SMS-level status code and how to fix it.", {
        code: z
            .number()
            .int()
    ...
    server.tool("get_sample_payload", "Get a sample JSON request body and the expected response payload for a specific ISMSPLUS endpoint.", {
        endpoint: z
            .enum(ENDPOINT_IDS)
    ...
    server.tool("validate_msisdn", "Validate and normalise a Bangladesh mobile phone number to the format required by the ISMSPLUS API (no API call is made).", {
        number: z
            .string()
    ...
    server.tool("get_integration_checklist", "Get a step-by-step checklist for integrating the SSL Wireless ISMSPLUS API into your application.", {}, async () => {
        const lines = [
